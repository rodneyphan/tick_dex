  ...
  Widget build (BuildContext context) {
    return Scaffold(
        appBar: AppBar( backgroundColor: Colors.yellowAccent,
          title: const Text("Tick-Borne Illnesses in the US", style: TextStyle( color: Colors.brown) ),
        ),
        body: Padding(
  ...
  Widget buildListBody(BuildContext context, int index) {
    return Row(
      ...
      children: <Widget> [
        SizedBox( width: 380,
          child: ListTile(
              title: Text(listOfDiseases[ index ].disName, textScaleFactor: 1.3, style: const TextStyle( color: Colors.brown) ),
              subtitle: Text( "  Cases in US for 2019:  " + listOfDiseases[index].disCasesPerYear, style: const TextStyle( color: Colors.green ) ),
            ),
        ),
        ElevatedButton(
          child: const Text("Go to a website"),
